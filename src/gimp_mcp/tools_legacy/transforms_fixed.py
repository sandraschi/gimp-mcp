...
class TransformTools(BaseToolCategory):
    ...
    async def _resize_image_impl(
        ...
        try:
            ...
            return self.create_error_response(f"Resize operation failed: {e!s}")

    async def _open_gimp_gui_impl(self, file_paths: list[str], comparison_mode: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            return self.create_error_response(f"Failed to open GIMP GUI: {e!s}")
    ...
    def register_tools(self, app: FastMCP) -> None:
        ...
        async def resize_image(
            ...
            """
            return await self._resize_image_impl(input_path, output_path, width, height, maintain_aspect, interpolation)

        @app.tool()
        async def open_gimp_gui(file_paths: list[str], comparison_mode: bool = False) -> dict[str, Any]:
            """
        ...
        async def compare_images_in_gimp(original_path: str, processed_path: str) -> dict[str, Any]:
            ...
            """
            return await self._open_gimp_gui_impl([original_path, processed_path], comparison_mode=True)
