...
class FilterTools(BaseToolCategory):
    ...
    async def _apply_blur_impl(
        ...
        try:
            ...
            valid_methods = ["gaussian", "motion", "zoom", "pixelize", "tile", "lens", "selective_gaussian"]
            if method not in valid_methods:
                return self.create_error_response(f"Invalid blur method. Must be one of: {', '.join(valid_methods)}")
...
"""
            ...
            try:
                ...
                return self.create_error_response(f"GIMP blur execution failed: {e!s}")

        except Exception as e:
            logger.error(f"Blur operation failed for {input_path}: {e}")
            return self.create_error_response(f"Blur operation failed: {e!s}")
