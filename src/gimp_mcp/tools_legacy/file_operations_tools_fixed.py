...
class FileOperationTools(FileOperationBase):
    ...
    @tool(
        ...
        parameters={"name": {"type": "string", "default": "World", "description": "Name to include in the greeting"}},
    )
    async def test_tool(self, name: str = "World") -> dict[str, Any]:
        ...
        """
        return {"greeting": f"Hello, {name}!", "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
