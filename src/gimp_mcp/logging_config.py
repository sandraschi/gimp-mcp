...
def setup_logging(log_level: str = "INFO", log_file: Path | None = None, component: str = "gimp_mcp") -> "logger":
    """
...
def log_operation_start(operation: str, **kwargs) -> dict[str, Any]:
    ...
    """
    context = {"operation": operation, "start_time": datetime.utcnow().isoformat(), "parameters": kwargs}
...
def log_tool_call(tool_name: str, parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    context = {"tool": tool_name, "start_time": datetime.utcnow().isoformat(), "parameters": parameters}
...
def log_server_start(port: int | None = None, mode: str = "stdio") -> None:
    """Log server startup."""
    logger.info(f"GIMP MCP Server starting in {mode} mode", extra={"server_start": {"mode": mode, "port": port}})


def log_server_stop(reason: str = "shutdown") -> None:
    """Log server shutdown."""
    logger.info(f"GIMP MCP Server stopping: {reason}", extra={"server_stop": {"reason": reason}})
...
def log_gimp_detection(gimp_path: str | None, success: bool) -> None:
    """Log GIMP detection result."""
    if success and gimp_path:
        logger.info(f"GIMP detected at: {gimp_path}", extra={"gimp_detection": {"path": gimp_path, "found": True}})
    else:
        logger.warning("GIMP not found - running in limited mode", extra={"gimp_detection": {"found": False}})
