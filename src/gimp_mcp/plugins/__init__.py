...
class PluginManager:
    ...
    def __init__(self, cli_wrapper: GimpCliWrapper, config: GimpConfig):
        ...
        self.cli_wrapper = cli_wrapper
        self.config = config
        self.plugins: dict[str, GimpPlugin] = {}
        self.logger = logging.getLogger(__name__)

    def load_plugins(self, plugin_dirs: list[str] | None = None) -> None:
    ...
    def register_tools(self, app) -> None:
        ...
        for plugin_name, plugin in self.plugins.items():
            try:
                ...
                self.logger.error(f"Error registering tools from plugin {plugin_name}: {e}", exc_info=True)

    def get_plugin(self, plugin_name: str) -> GimpPlugin | None:
