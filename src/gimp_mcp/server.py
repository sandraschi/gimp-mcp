...
class GimpMcpServer:
    ...
    def _register_core_plugins(self) -> None:
        ...
        for plugin_class in CORE_PLUGINS:
            try:
                ...
                self.logger.error(f"Failed to register core plugin {plugin_class.__name__}: {e}", exc_info=True)
    ...
    def _initialize_help_tools(self) -> None:
        ...
        if not hasattr(self, "help_tools"):
            self.help_tools = HelpTools(self.interaction_manager, self.config, tool_categories=tool_categories)
