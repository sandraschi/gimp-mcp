...
try:
    ...
    print('✅ FileOperationTools instance created')
    ...
    if hasattr(mcp, '_tools') and mcp._tools:
        print(f'✅ Found {len(mcp._tools)} registered tools: {list(mcp._tools.keys())}')
    elif hasattr(mcp, 'tools') and mcp.tools:
        print(f'✅ Found {len(mcp.tools)} registered tools: {list(mcp.tools.keys())}')
    else:
