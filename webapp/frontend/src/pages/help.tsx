...
import { Blocks, type Cog, Cpu, FileCode, Globe, Package, Plug, Terminal } from "lucide-react";
...
function TabContent({ children, id }: { children: React.ReactNode; id: string }) {
  return (
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return <h3 className="text-lg font-semibold text-foreground mt-6 mb-3 first:mt-0">{children}</h3>;
}
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">GIMP MCP Server v4.0.0 — FastMCP 3.2 SOTA</p>
      </div>
...
function InstallContent() {
  return (
    <div className="space-y-4 text-sm text-muted-foreground">
      <p>Follow these steps to set up the GIMP MCP Server with FastMCP 3.2 and the Fleet Standard web application.</p>

      <SectionHeading>Prerequisites</SectionHeading>
      <ul className="list-disc list-inside space-y-1 text-sm">
        <li>
          <strong className="text-foreground">GIMP 3.2+</strong> — standalone install (NOT Windows Store; CLI batch mode
          is unavailable in the Store version)
        </li>
        <li>
          <strong className="text-foreground">Python 3.12+</strong> — required for FastMCP 3.2
        </li>
        <li>
          <strong className="text-foreground">uv</strong> — recommended for high-performance dependency management
        </li>
      </ul>

      <SectionHeading>1. Install GIMP 3.2+</SectionHeading>
      <p>
        Download GIMP 3.2+ from <span className="text-primary">gimp.org</span>. Use the standalone Windows installer
        (not the Microsoft Store version). Default install path:
      </p>
...
uv sync`}
      ...
      <p>
        Create a <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">config.yaml</code> to manually
        point to your GIMP executable if not auto-detected:
      </p>
      <CodeBlock code={`gimp_executable: "C:\\\\Program Files\\\\GIMP 3\\\\bin\\\\gimp-console-3.0.exe"`} />
...
just serve        # backend only (reload on changes)`}
      ...
      <ul className="list-disc list-inside space-y-1 text-sm">
        <li>
          Open <span className="text-primary">http://localhost:10772</span> in your browser
        </li>
        <li>Check the Dashboard for live port status</li>
        <li>Visit Tools Explorer to verify all 17 portmanteau tools are listed</li>
        <li>
          Check bridge status:{" "}
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">just bridge-status</code>
        </li>
      </ul>
    </div>
  );
...
function ArchitectureContent() {
  return (
    <div className="space-y-4 text-sm text-muted-foreground">
      <p>GIMP MCP follows a multi-layer architecture with clear separation of concerns:</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
        {[
          ...
          return (
            <div key={layer.label} className="bg-secondary/20 rounded-xl p-4 border border-border/30 space-y-2">
      ...
      <p>
        Instead of 50+ individual tools, GIMP MCP consolidates operations into 9 portmanteau tools. Each tool accepts an{" "}
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">operation</code> parameter to select the
        specific action. This reduces context overhead for LLMs while maintaining full GIMP capability.
      </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-2">
        {[
          ...
          <div key={name} className="bg-secondary/30 px-3 py-2 rounded-lg text-xs font-mono flex justify-between">
      ...
      <ul className="list-disc list-inside space-y-1 text-sm">
        <li>
          <strong className="text-green-400">Live Bridge</strong> — TCP connection to a running GIMP 3 instance for
          real-time operations
        </li>
        <li>
          <strong className="text-blue-400">Headless CLI</strong> — Batch mode via{" "}
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">gimp-console-3.exe</code> for automated
...
function GimpBridgeContent() {
  return (
    <div className="space-y-4 text-sm text-muted-foreground">
      <p>
        The GIMP Bridge enables real-time bidirectional communication between the MCP server and a running GIMP 3.2+
        instance.
      </p>
      ...
      <ol className="list-decimal list-inside space-y-2 text-sm">
        <li>
          ...
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">gimp_mcp_bridge.py</code>) runs inside
          GIMP and starts a TCP server on port 10824
        </li>
        <li>
          The <strong className="text-foreground">bridge wrapper</strong> (
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">bridge_wrapper.py</code>) in the MCP
          server connects to this TCP server
        </li>
        <li>
          Python code snippets are sent as JSON over TCP and executed on GIMP's main thread via GLib idle callbacks
        </li>
      ...
      <p>
        The <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">gimp_pdb</code> tool is a universal
        escape hatch that can call any GIMP PDB procedure by name. This gives you access to GIMP's full ~1000-procedure
        library:
      </p>
      <CodeBlock
...
Get-NetTCPConnection -LocalPort 10824`}
      />
      <p>The Dashboard page also shows bridge status with a green indicator when connected.</p>
    </div>
  );
}
...
function PluginsContent() {
  return (
    <div className="space-y-4 text-sm text-muted-foreground">
      <p>GIMP MCP includes a bridge plugin that runs inside GIMP itself, enabling real-time AI control.</p>

      <SectionHeading>Bridge Plugin</SectionHeading>
      <p>
        The main bridge plugin is{" "}
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">gimp_mcp_bridge.py</code> — a
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">Gimp.PlugIn</code> subclass that:
      </p>
      <ul className="list-disc list-inside space-y-1 text-sm mt-2">
        <li>
          Starts a TCP listener on <strong className="text-foreground">127.0.0.1:10824</strong>
        </li>
        <li>
        ...
        <li>
          Provides <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">Gimp</code>,{" "}
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">pdb</code>,{" "}
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">GLib</code> globals to executed
          snippets
        </li>
        <li>
          Sends back <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">{`{"result": "..."}`}</code>{" "}
          or <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">{`{"error": "..."}`}</code>
        </li>
      </ul>
...
# %APPDATA%\\GIMP\\3.0\\plug-ins\\gimp_mcp_bridge\\`}
      ...
      <CodeBlock code={`<Image> / Filters / Development / MCP / Start MCP Bridge`} />

      <SectionHeading>Plugin Architecture</SectionHeading>
      <p>
        The bridge plugin extends{" "}
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">Gimp.PlugIn</code> and follows GIMP 3's
        plugin protocol:
      </p>
      <ul className="list-disc list-inside space-y-1 text-sm">
        <li>
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">do_query_procedures()</code> —
          registers
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">gimp-mcp-bridge-start</code>
        </li>
        <li>
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">do_create_procedure()</code> — defines
          the procedure metadata and menu location
        </li>
        <li>
          <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">run()</code> — starts the background
          TCP listener thread and GLib idle polling loop
        </li>
      </ul>
      ...
      <p>
        ...
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">GimpToolPlugin</code>. Create a Python
        class inheriting from{" "}
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">GimpToolPlugin</code>, implement{" "}
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">register_tools(app)</code>, and place it
        in the plugins directory. Tools are auto-discovered at startup.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4 text-sm text-muted-foreground">
      ...
      <p>
        ...
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">gimp-console-3.exe</code> without opening
        the GUI:
      </p>
      <CodeBlock
...
just test-pdb`}
      ...
      <p>The backend (port 10773) exposes these REST endpoints via FastMCP's HTTP transport:</p>
      ...
      <p>
        ...
        <code className="text-xs bg-secondary px-1.5 py-0.5 rounded font-mono">justfile</code> for all common
        operations:
      </p>
