...
# Bundle .env.example into installer for first-run setup
...
if (Test-Path $envExampleSrc) {
    ...
    Write-Host "  WARNING: No .env.example at repo root - create one" -ForegroundColor DarkYellow
}
...
$strayExe = "$PSScriptRoot\target\release\gimp-mcp-backend.exe"
